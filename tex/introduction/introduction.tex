...
Hybrid materials \index{hybrid materials} based on FunzioNano are multifunctional nanoparticles which can provide significant benefit in EOR. FunzioNano with blocked functional groups can be used as a latent cross-linker for polymers which are made from renewable resources, e.g. cellulose based polymers. 

FunzioNano cross-linker and polymer are injected as a formulation at the same time. Hydrolysis within the requested time window leads to activation of the functional groups and thereafter fast and efficient gelation by cross-linking \index{cross-linker} of FunzioNano and polymer, e.g. through hydrolysis \index{hydrolysis} of fully functionalized nanomaterial on partially hydrolyzied polyacrylamide. 
